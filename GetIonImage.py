...
# def ticImage(filename,t2m,hdr):
#     '''
#     Function will create tic image
#     '''
#     mass1, mass2 = min(t2m), max(t2m) 
#     id = (t2m >= mass1) & (t2m <= mass2)
#     bytes, endian = 2, 'H'
...
#                     continue 
#                 test = struct.unpack(endian,f.read(bytes))[0]
#                 spcarr.append(np.float32(test))
#             datamatrix[r,c] = sum(spcarr/sum(spcarr))
#     return(datamatrix)
...
## batch process stuff ...
def processMSIBatch(massrange=[], mfiltrad = 3, msk_tic = False, msk_mim = False):
    '''
...
def main():    
    ...
    processMSIBatch(massrange = args.massrange, mfiltrad = args.mfiltrad, msk_tic = args.msk_tic, msk_mim=args.msk_mim)
