...
## batch process stuff ...
def processMSIBatch(massrange=[], mfiltrad = 3, msk_tic = False, msk_mim = False, massrange_std = False):
    '''
    ...
    for f in hdrs:
        ...
        if (len(massrange) == 4):
	       print('length of massrange : %s' %len(massrange))
...
def main():    
    parser = argparse.ArgumentParser(description="Create an extraced ion image and its corresponding binary mask")
    parser.add_argument('-mz',dest = "massrange",type = float, nargs ='+', default=[],help = "Desired m/z range, if mass length is 6 we assume inital 2 mass range corresponds to the drug, then tissue and std respectively")
    parser.add_argument('-mz_std',dest = "massrange_std",type = float, nargs ='+', default= False,help = "describe standard mass range")
    ...
    processMSIBatch(massrange = args.massrange,massrange_std = args.massrange_std ,mfiltrad = args.mfiltrad, msk_tic = args.msk_tic, msk_mim=args.msk_mim)
